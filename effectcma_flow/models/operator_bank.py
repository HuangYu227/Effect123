...
class TemporalSegmentExpert(nn.Module):
    """Velocity expert whose computation is constrained to the time dimension."""
    ...
    def forward(self, h: torch.Tensor, t_code: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        h, scale, shift = self.ada_norm(h, t_code)
        x = h.permute(0, 2, 3, 1).reshape(batch * channels, hidden, length)
...
class ChannelInteractionExpert(nn.Module):
    ...
    def forward(self, h: torch.Tensor, t_code: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        batch, length, channels, hidden = h.shape
        h, scale, shift = self.ada_norm(h, t_code)
        x = h.reshape(batch * length, channels, hidden)
        attended, attn = self.channel_attn(x, x, x, need_weights=True, average_attn_weights=False)
        y = attended + self.ffn(attended)
        out = self.head(y).reshape(batch, length, channels)
        aux = {
            "channel_attn_entropy": _entropy(attn.detach().mean(dim=1), dim=-1).mean(),
...
class FrequencyBandExpert(nn.Module):
    ...
    def forward(self, h: torch.Tensor, t_code: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        for mask in bands:
            mask = mask[None, None, :]
            filtered = torch.fft.irfft(freq * mask, n=length, dim=-1)
            components.append(filtered.to(x.dtype))
            energy.append((freq.abs().square() * mask).mean(dim=(1, 2)))
